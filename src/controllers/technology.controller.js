...
exports.getById = async (req, res, next) => {
  try {
    ...
    if (!technology) return res.status(404).json({ message: 'Tecnologia não encontrada' });
...
exports.destroy = async (req, res, next) => {
  try {
    ...
    if (!deletedCount) return res.status(404).json({ message: 'Tecnologia não encontrada' });
